...
新增了将优化成果量化保存到本地文件的功能：

**导出格式：**
1. **JSON 格式** - 机器可读的结构化数据，包含完整聚合统计和所有详细记录
2. **Markdown 格式** - 人类可读的报告，包含表格化的聚合统计和详细记录列表

**导出位置：**
- 文件保存在应用的外部存储目录：`/Android/data/com.example.myapplication/files/`
- 文件名带时间戳：`playback_metrics_YYYY-MM-DD_HH-mm-ss.json` 和 `playback_metrics_report_YYYY-MM-DD_HH-mm-ss.md`

**使用方式：**
1. 进入「起播指标」页面
2. 点击右上角「📤 导出」按钮
3. Toast 提示导出成功，显示文件完整路径
...
4. 点击「📤 导出」将数据保存到本地文件
5. 记录 avgColdStartMs / avgPreloadMs / avgImprovementPct / preloadHitRate
```
